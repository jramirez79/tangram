...
void main() {
    ...
    vec4 position = vec4(a_position.xy, a_position.z / TANGRAM_HEIGHT_SCALE, 1.); // convert height back to meters
        ...
        // float offset_scale = 1.; // don't use offset if this is set to 1 – only scale width
        ...
        width += 200. * sin(u_time) * offset_scale;
        ...
        // vec2 offset = perp * width * (1. - offset_scale);
        vec2 offset = normal * width * (1. - offset_scale);
        offset += 200. * sin(u_time);
